...
\articletype{Preprint Manuscript}% Specify the article type or omit as appropriateSubmission to \emph{Building Research and Information} Journal

\title{SpaceMatch: Using ecological momentary assessment (EMA) to match occupants to suitable flexible workspaces}
...
The flexible workplace paradigm for buildings is becoming mainstream for commercial building offices; in these spaces, occupants are given a choice of spaces to do their work and personal activities on a day-to-day basis. This workplace strategy drives down rental costs and cost per occupant by improving spatial utilization; but studies have shown that its impact on occupant satisfaction is not always positive. One way to characterize this issue is to increase the frequency and volume of occupant feedback in from buildings to better understand occupant needs in this context. However, frequent collection of occupant preferences using traditional methods such as surveys, interviews presents significant challenges of scalability among others. This paper showcases \emph{SpaceMatch} - a web based platform for easy collection of individual comfort preferences with the goal of matching occupants to suitable environmentally comfort \& spatially efficient flexible workspaces. This new approach connects occupants with a catalog of available work desks using a web based mobile application and enables them to easily provide environmental feedback in real-time. It also facilitates operators by merging this feedback data with indoor environmental values, from Internet-of-Things (IoT) sensors, to optimize space and energy use by grouping occupants with similar preferences. This paper outlines a pilot implementation of the initial version of the SpaceMatch platform at the School of Design and Environment (SDE) at the National University of Singapore (NUS). This pilot implementation collected 1182 responses from 25 field-based research participants over a 30 days study. From this initial data set, the results show that the flexible-based work occupants can be segmented into specific \emph{types} of users based on their accumulated preference data. This paper illustrates an overview of the development methodology, challenges and results from the pilot implementation, and next steps towards training a recommendation engine model to assign occupants to workspaces.
...
Researchers increasingly adapt data driven methodologies to address challenges of occupant satisfaction, environmental quality and energy efficiency in buildings today \cite{Textmining, occupantcomplaints, IEQandsatisfaction}. This study uses data driven methods to identify personalized comfort profiles of users - clustering users into \emph{types} based on similar environmental preferences as shown in section \ref{Section:Results}. Such results could be useful in multiple ways; for one, grouping users with similar environmental preferences could improve occupant comfort and space \& energy efficiency as shown by other studies \cite{KAMARULZAMAN2011262}. Next, this method could also enhance the feedback given to designers and operators about future building design features and operating strategies to improve flexible workplace occupant satisfaction and performance \cite{KWON2019356}.In parallel, its also easy to see how the same methodology could be used to distinguish spaces based on occupant comfort feedback data and IoT data - to derive comfort profile types of spaces.
